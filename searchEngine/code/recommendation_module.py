...
class RecommendationModule:
    ...
    def construct_dt_matrix(self, files, topK=200):  # 创建每个文件中的关键词权重矩阵
        ...
        for i in files:
            ...
            tags = jieba.analyse.extract_tags(title + '。' + body, topK=topK, withWeight=True)  # 计算出权重
            cleaned_dict = {}
            for word, tfidf in tags:
                word = word.strip().lower()  # 使用 strip()方法去除字符串两端的空格，并用 lower()方法将字符串转换为小写
                if word == '' or self.is_number(word):
                    continue
                cleaned_dict[word] = tfidf  # 如果关键词不为空且不是数字，则将其及其 tf-idf 权重添加到 cleaned_dict 字典中。
        ...
        for docid, t_tfidf in dt:
            dt_matrix[i][0] = docid  # 这里的第一个记录的是文件id
            for term, tfidf in t_tfidf.items():
                dt_matrix[i][terms[term]] = tfidf  # 记录某个文件中各个词语的权重
            i += 1
    ...
    def find_k_nearest(self, k, topK):
        ...
        self.construct_k_nearest_matrix(dt_matrix, k)  # 这里有问题
